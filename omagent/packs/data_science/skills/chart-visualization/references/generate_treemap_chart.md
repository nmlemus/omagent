...
## Functional Overview
Displays hierarchical data as a set of nested rectangles, with areas proportional to values.

## Input Fields
### Required
- `data`: array<object>, node array where each contains `name` (string) and `value` (number). Can nest `children` recursively.
