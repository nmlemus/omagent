...
## Functional Overview
Displays relationships between two continuous variables, helping identify correlations or clusters.

## Input Fields
### Required
- `data`: array<object>, each record containing `x` (number) and `y` (number), optional `group` (string).
