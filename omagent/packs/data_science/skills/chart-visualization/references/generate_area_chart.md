...
## Functional Overview
Displays the trend of accumulated values over time or across categories. Suitable for showing volume and proportion trends.

## Input Fields
### Required
- `data`: array<object>, each containing `time` (string) and `value` (number). For stacked areas, include `group` (string). Requires at least 1 record.
