...
## Functional Overview
Visualizes the flow of data or resources between nodes, with flow width proportional to the quantity.

## Input Fields
### Required
- `data`: array<object>, each record containing `source` (string), `target` (string), and `value` (number).
